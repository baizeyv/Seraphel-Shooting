package net.mwplay.cocostudio.ui.base.plist;
	...
	public static class PlistAtlasData {
		...
		public PlistAtlasData(FileHandle packFile, FileHandle imagesDir, TextureFilter minFilter, TextureFilter maxFilter) {
			try {
                ...
                pageImage.textureFile = file;
                pageImage.width = textureWidth;
                pageImage.height = textureHeight;
                pageImage.useMipMaps = min.isMipMap();
                pageImage.format = format;
                pageImage.minFilter = min;
                pageImage.magFilter = max;
                pageImage.uWrap = repeatX;
                pageImage.vWrap = repeatY;
	...
	public NinePatch createPatch (String name) {
		for (int i = 0, n = regions.size; i < n; i++) {
			...
			if (region.name.equals(name)) {
				int[] splits = region.values[0];
				if (splits == null) throw new IllegalArgumentException("Region does not have ninepatch splits: " + name);
				NinePatch patch = new NinePatch(region, splits[0], splits[1], splits[2], splits[3]);
				if (region.values[1] != null) patch.setPadding(region.values[1][0], region.values[1][1], region.values[1][2], region.values[1][3]);
				return patch;
			}
		}
		return null;
